...
public class Handler 
{
    ...
    private String destinationPrefix = System.getenv("dest_prefix"); //"dest_key" is one environment key set at Lambda runtime
	...
	public void handleRequest(S3Event event, Context context) {
	    ...
	    //
	    //   bucket:  cfsrc                                          -> cfdest
	    //   key:     prefix/EP4CMY7FDJR6Y.2017-06-28-16.ea6de06c.gz -> destprefix/yyyy=2017/mm=06/dd=28/hh=16/EP4CMY7FDJR6Y.2017-06-28-16.ea6de06c.gz
	    //
	    
	    String regex   = "(?<prefix>.*)\\/(?<distributionid>.*)\\.(?<year>[0-9]{4})\\-(?<month>[0-9]{2})\\-(?<day>[0-9]{2})\\-(?<hour>[0-9]{2})\\.(?<hash>.*)\\.gz";
	    String replace = "yyyy=${year}/mm=${month}/dd=${day}/hh=${hour}/${distributionid}.${year}-${month}-${day}-${hour}.${hash}.gz";
	    String destinationKey = destinationPrefix +"/" +sourceKey.replaceAll(regex, replace);
